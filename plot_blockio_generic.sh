...
#########################################################################################
# ORNL OLCF-3 generic plot block I/O benchmark results script	                        #
#                                                                                       #
#########################################################################################
...
#                                                                                                               #
# Usage:                                                                                                        #
# sh ./<this-script> <blockio-data-file-name(CSV)> <appliancename> <diskname>		                        #
#                                                                                                               #
# Example:                                                                                                      #
# sh ./plot_blockio_generic.sh block_io_single_host_scale_up_Oct_01_11_11_31.csv acme-mk1 sas-10k		#
#                                                                                                               #
...
let last_plot_index=block_size_len*queue_size_len


#to set the xtics correctly later on
for b in ${device_num[@]}; do xtics="$xtics,$b"; done; xtics=${xtics#,}

for mode in ${io_mode}; do
	temp_out_file="${mode}_temp.out"
	[ -e ${temp_out_file} ] && rm ${temp_out_file}
	for device in ${device_num}; do
		outstring="${device}"
		for block in ${block_size}; do
			for queue in ${queue_size}; do
				tempstring=$(grep "${mode},${block},${queue},${device}," ${temp_file} | awk 'BEGIN { FS = "," } ; {print $5,$7,$9}')
				outstring="${outstring}	${tempstring}"
				unset tempstring
				#grep "${mode},${block},${queue},${device}" ${temp_file} | awk 'BEGIN { FS = "," } ; {print $4,$5,$6,$7,$9}' >> ${temp_out_file}
			done
		done
	...
	#for queue in ${queue_size}; do
		...
		for queue in ${queue_size}; do
			...
			if [ ${gnp_index} -eq "1" ]; then
				plot_command_first_max="plot \"${temp_out_file}\" using 1:${first_plot_index} title \"Max-${queue}-${block}\" with linespoints," 
				plot_command_max="${plot_command_max} ${plot_command_first_max}"
			elif [ ${gnp_index} -eq "${last_plot_index}" ]; then
                                plot_command_last_max="\"${temp_out_file}\" using 1:${first_plot_index} title \"Max-${queue}-${block}\" with linespoints" 
				plot_command_max="${plot_command_max} ${plot_command_last_max}"
   			else
				plot_command_regular_max="\"${temp_out_file}\" using 1:${first_plot_index} title \"Max-${queue}-${block}\" with linespoints,"
				plot_command_max="${plot_command_max} ${plot_command_regular_max}"
			fi


			if [ ${gnp_index} -eq "1" ]; then
                                plot_command_first_median="plot \"${temp_out_file}\" using 1:${second_plot_index}:${third_plot_index} title \"Median-${queue}-${block}\" with errorlines,"
                                plot_command_median="${plot_command_median} ${plot_command_first_median}"
                        elif [ ${gnp_index} -eq "${last_plot_index}" ]; then
                                plot_command_last_median="\"${temp_out_file}\" using 1:${second_plot_index}:${third_plot_index} title \"Median-${queue}-${block}\" with errorlines"
                                plot_command_median="${plot_command_median} ${plot_command_last_median}"
                        else
                                plot_command_regular_median="\"${temp_out_file}\" using 1:${second_plot_index}:${third_plot_index} title \"Median-${queue}-${block}\" with errorlines,"
